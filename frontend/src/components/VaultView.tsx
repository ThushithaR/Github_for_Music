...
// Generate stable waveform heights (client-only, seeded)
function genCaptureBarHeights(count: number): number[] {
  const heights: number[] = [];
  let seed = 42;
  for (let i = 0; i < count; i++) {
    seed = (seed * 1664525 + 1013904223) >>> 0;
    const rand = ((seed >>> 16) & 0xFFFF) / 65535;
    heights.push(rand * 15 + 2);
  }
  return heights;
}
...
export default function VaultView() {
  ...
  // Stable client-side waveform heights to avoid SSR/client hydration mismatch
  const [captureBarHeights, setCaptureBarHeights] = useState<number[]>(() => Array(40).fill(8));
  useEffect(() => {
    setCaptureBarHeights(genCaptureBarHeights(40));
  }, []);
    ...
    <div className="view active" id="view-vault">
      <div id="capture-bar">
        ...
        <div id="capture-waveform">
          {captureBarHeights.map((h, i) => (
            <div key={i} className="wv-bar" style={{height: `${h}px`, background: audio.isRecording ? '#E53E3E' : (audio.isListening ? 'var(--amber)' : 'var(--text-muted)')}}></div>
          ))}
        </div>
